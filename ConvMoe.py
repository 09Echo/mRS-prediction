...
class SparseDispatcher(object):
    ...
    def combine(self, expert_out, raw_size:Optional[Union[None,Tuple,List]],multiply_by_gates=True):
        ...
        if multiply_by_gates:
            stitched = stitched.mul(self._nonzero_gates.unsqueeze(2).unsqueeze(3))
...
class Conv_lora(nn.Module):
    ...
    def forward(self,x):
        ...
        x = F.interpolate(x,size=tuple(size0),mode="bilinear")
        x = self.conv(x)
...
class MoEConv(nn.Module):
    def __init__(self, d, scales:List[int], M=4, K=1, noisy_gating=True):
        """Constructor
        Args:
    ...
    def forward(self, feats, loss_coef=1e-2, noise_epsilon=1e-2):
        ...
        feats = feats.permute(0,3,1,2).contiguous()   #BCHW
        ...
        if self.noisy_gating and self.training:
            ...
            noisy_logits = clean_logits + (torch.randn_like(clean_logits) * noise_stddev) #torch.randn_like用于生成一个与 clean_logits 张量具有相同形状的张量，其中的元素是从标准正态分布（均值为0，标准差为1）中随机采样的。
            logits = noisy_logits
        else:
            logits = clean_logits
