...
class BasicBlock(nn.Module):
    expansion = 1 #残差结构中主分支采用的卷积核个数是否发生变化
    #downsample下采样，stride=2，高宽减半，虚线残差结构要用
    #使用BN时，不使用偏置
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:   #残差维度不匹配时
...
class ResNet(nn.Module):
    #blocks_num所使用残差结构的数目，例如34层的为[3,4,6,3]，num_classes训练集的分类个数
    def __init__(self,block,blocks_num,num_classes=1000,include_top=True,groups=1,width_per_group=64):
        super(ResNet, self).__init__()
        self.include_top = include_top
        self.in_channel = 64 #3*3max poooling 后的输入

        self.groups = groups
        self.width_per_group = width_per_group

        self.conv1 = nn.Conv2d(1, self.in_channel, kernel_size=7, stride=2,
                               padding=3, bias=False) #输入通道为1
        self.bn1 = nn.BatchNorm2d(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, blocks_num[0]) #stride=1是因为第一层之前的池化层已经将高宽减半
    ...
    def _make_layer(self, block, channel, block_num, stride=1):  #channel是指每一层block的第一个卷积核的个数，block_num指该层一共包含了多少个残差结构
        downsample = None
