...
class DeepSymNet2d(nn.Module):
    def __init__(self, num_classes=2, in_ch=1):
        super(DeepSymNet2d, self).__init__()

        self.conv1 = BasicConv2d(in_channels=in_ch, out_channels=64, kernel_size=7, stride=2, padding=3)
        self.maxpool1 = nn.MaxPool2d(3, stride=2, ceil_mode=True)
        ...
        self.inception2b = Inception(512, 256, 160, 320, 32, 128, 128)
        self.maxpool4 = nn.MaxPool2d(3, stride=2, ceil_mode=True)
        # 7x7x512

        self.conv1x1 = nn.Conv2d(in_channels=832, out_channels=832, kernel_size=1, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(832)
    ...
    def forward(self, x):
        x_left = x[:, :, :, 0:112]
        x_right = x[:, :, :, 112:224]
        x_right_flip = torch.flip(x_right, dims=[3])
        ...
        # 对绝对值操作后输入-
        l1_diff = F.relu(self.bn1(self.conv1x1(l1_diff)))
