...
class Prediction(nn.Module):
    def __init__(self, pretrained=''):
        super().__init__()
        self.model = blip_feature_extractor(pretrained=pretrained)
    ...
    def forward(self, image, text, mode):
        feature = self.model(image, text, mode=mode)
        feature_average = torch.mean(feature, dim=1)
        out = self.fc(feature_average)
        return out
...
class BLIP_Base(nn.Module):
    ...
    def forward(self, image, caption, mode):
        ...
        text = self.tokenizer(caption, max_length=512,return_tensors="pt",padding='max_length',truncation=True).to(image.device)
...
def init_tokenizer():
    tokenizer = BertTokenizer.from_pretrained('/home/hubin/Codes/ISLES2024/big_model/LLM/LLM_huggingface/bert')
...
if __name__ == '__main__':
    ...
    y = model(x, text, mode='multimodal')
    print(y.shape)
