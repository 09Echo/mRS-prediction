...
class ProVe():
    def __init__(self):
        ...
        self.label = pd.read_excel(data_info_path, sheet_name="LLM_75")['label'].values
        self.data = pd.read_excel(data_info_path, sheet_name="LLM_75")[
...
def extern_test():
    weights = []
    path = '/home/hubin/Codes/ISLES2024/image_text_models/savemodels/BLIP/'

    txt_path = '/home/hubin/Codes/ISLES2024/big_model/print/external_results_75'
    txt_result_path = os.path.join(txt_path, 'BLIP')
    for filename in os.listdir(path):
        if filename.endswith(".pth"):
            weights.append(path + filename)

    weights = [path + '2fold_19epoch_72.00%acc_0.59%f1_0.529%loss.pth'] #blip
    ...
    for ki in range(len(weights)):
        ...
        model = Prediction(pretrained='./model_base.pth').cuda()
        # load model weights
        ...
        with torch.no_grad():
            for step, test_data in enumerate(test_dl):
                ...
                outputs = model(test_images, test_patient_text, mode = 'multimodal')  # 输出值，有正有负 #维度从【2】变成【1，2】
    ...
    text.to_csv(os.path.join(path, "index_external_75.csv"), index=None, encoding='utf8')
